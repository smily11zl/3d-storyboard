...
1. **生成端 skill 改造**：AI 生成「相机对象 + 多段组合」
   ...
   - 简单运动 = 2 个关键帧 pose（S，可编辑）；复杂运动 = 3+ pose（C，自由）
...
3. **后端识别逻辑**：读相机 NLA strips → 提取段 → 按 pose 数标 S/C → 按 start_time 排序（不再判 sequence/parallel）
...
- **编辑简单段（S）**：从当前视角设相机 / TransformControls 拖拽相机 / 数值面板（位置·朝向·FOV）/ 两 pose 插值（缓动 + 起幅落幅停留）/ 硬切·连续衔接
