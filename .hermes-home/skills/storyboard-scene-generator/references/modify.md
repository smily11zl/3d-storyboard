...
**【禁止从零重建——必须严格遵守】** 情况 B 下**禁止从头重新生成整个场景**。现有 blend 里可能有脚本无法复现的手动编辑（材质微调、骨骼姿势、约束参数等），从零重建会丢失这些。必须 `bpy.ops.wm.open_mainfile` 打开现有 blend，只做增量修改，最后 `save_as_mainfile` 输出新版本 blend。**全程不写、不新建任何 script_vN.py。**
